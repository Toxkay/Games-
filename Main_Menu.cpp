...
int getChoice() {
    int choice;
    while (true) {
        cout << "\nEnter your choice (1-6, 8/9): ";
        cin >> choice;

        if (cin.fail() || (choice < 1 || (choice > 6 && choice < 8) || choice > 9)) {
...
void playGame(int choice) {
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        // Game 8 implementation
    } else if (choice == 9) {
        cout << "Starting SUS...\n";
        // Game 9 implementation
    } else {
        cout << "Invalid game choice. Please restart the game.\n";
    }
}
